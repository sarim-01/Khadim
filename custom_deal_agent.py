...
class CustomDealAgent:
    ...
    def _calculate_quantities(self, items_list, num_people):
        """
        Calculate intelligent quantities for each item based on:
        1. Item's serving size (how many people does 1 unit serve)
        2. Number of people in the deal
        3. Item category (drinks per person, mains shared, etc.)
        
        Returns: list of (item_dict, quantity) tuples
        """
        items_with_qty = []
        
        for item in items_list:
            item_category = item.get('item_category', 'main').lower()
            serving_size = int(item.get('serving_size') or 1)  # How many people does 1 serve
            
            # Calculate quantity based on category and serving size
            if item_category == 'drink':
                # Drinks: 1 per person (or 1 per 2 people depending on preference)
                qty = max(1, num_people)
            elif item_category == 'bread':
                # Bread: Roughly 1 bread per 1-2 people
                qty = max(1, (num_people + 1) // 2)
            elif item_category == 'side':
                # Sides: 1 side can typically serve 2-3 people, adjust based on serving_size
                qty = max(1, (num_people + serving_size - 1) // serving_size)
            elif item_category == 'appetizer':
                # Appetizer: Share between people (1 per 2 people)
                qty = max(1, (num_people + 1) // 2)
            else:  # main, or any other category
                # Main dishes: Calculate based on serving size
                # If serving_size is 1, qty = num_people
                # If serving_size is 2, qty = num_people / 2
                # If serving_size is 4, qty = num_people / 4
                qty = max(1, (num_people + serving_size - 1) // serving_size)
            
            items_with_qty.append((item, qty))
            print(f"[DealAgent] Item '{item['item_name']}' (serves {serving_size}, category: {item_category}): qty={qty} for {num_people} people")
        ...
        try:
            ...
            for i in final_items:
                ...
                if orig_item:
                    ...
                    display_text = f"{i['item_name']}" if qty == 1 else f"{qty}× {i['item_name']}"
